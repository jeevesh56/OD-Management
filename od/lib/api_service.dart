...
// ── EVENTS ────────────────────────────────────────────────────────────────────
class EventsApi {
  static Future<ApiResult<List>> list() async {
    if (kUseMockApi) {
      ...
      for (final r in MockOdStore.items) {
        ...
        byName.putIfAbsent(name, () {
          return <String, dynamic>{
            'event_id': name, // mock-only
            'event_name': name,
            'organiser_body': r['organiser'],
            'venue': r['venue'],
  ...
  static Future<ApiResult<List>> registrations(String eventId) async {
    if (kUseMockApi) {
      final regs = MockOdStore.items
          .where((r) => (r['event_name'] as String? ?? '').trim() == eventId)
          .toList();
      return ApiResult.success(regs);
    }

    try {
      final res = await http
          .get(
            Uri.parse('$kBaseUrl/api/events/$eventId/registrations'),
            headers: AuthStore.headers,
          )
          .timeout(const Duration(seconds: 10));
      ...
      final body = jsonDecode(res.body);
      if (res.statusCode == 200) {
        return ApiResult.success(body['registrations'] as List);
      }
      return ApiResult.fail(body['error'] ?? 'Failed to load registrations');
    } catch (e) {
      return ApiResult.fail('Network error: $e');
    }
  }
